...
namespace Dot_Slash
{
	public class ColourDetector : Filter
	{
		...
		public void pump(ref AdvertDetails _advertDetails)
		{
			...
			{ 
				_advertDetails.Error +=";car not found";
				return;
			}
			Bitmap img = _advertDetails.Image.ToBitmap().Clone(_advertDetails.Rect, System.Drawing.Imaging.PixelFormat.Format32bppRgb);
			int width = img.Width;
			int height = img.Height;
			String[] colours = loopThroughPixels(colourBuckets, width, height, img);
			_advertDetails.Colour1 = colours[0];
			_advertDetails.Colour2 = colours[1];
			_advertDetails.Colour3 = colours[2];
			_advertDetails.Hex1 = colours[3];
			_advertDetails.Hex2 = colours[4];
			_advertDetails.Hex3 = colours[5];
		}
		private String[] loopThroughPixels(ArrayList colourBuckets, int width, int height, Bitmap img)
		{
